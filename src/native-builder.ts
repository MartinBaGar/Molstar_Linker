...
export const NativeBuilder = {
  ...
  async buildNativeScene(
    ...
    const modelStructure = presetResult.structure;

    // 3. Select a specific range of residues (e.g., 30 to 45)
    const mySelection = MS.struct.generator.atomGroups({
        'residue-test': MS.core.rel.inRange([MS.ammp('auth_seq_id'), 30, 45])
    });

    // 4. Create the component
    const surfaceComponent = await plugin.builders.structure.tryCreateComponentFromExpression(
        modelStructure,
        mySelection,
        'console-res-30-45',
