...
// ---------------------------------------------------------------------------
// Mol* and MvsBuilder are loaded as <script> tags in sandbox.html
// ---------------------------------------------------------------------------
declare const molstar: {
  Viewer: {
    create(elementId: string, options: Record<string, unknown>): Promise<MolstarViewer>;
  };
  PluginExtensions: {
    mvs: {
      MVSData: { fromMVSJ(json: string): unknown };
      loadMVS(
        plugin: unknown,
        data: unknown,
...
interface MolstarViewer {
  plugin: unknown;
  loadMvsData?(data: string, format: string): Promise<void>;
}
...
// ---------------------------------------------------------------------------
let viewerInstance: MolstarViewer | null = null;
