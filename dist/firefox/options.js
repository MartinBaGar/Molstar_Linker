...
function updateSubParamsDrawer(drawer, repType, storedValues = {}) {
    ...
    for (const [paramName, paramType] of Object.entries(schema)) {
        ...
        if (paramType === 'boolean') {
            ...
            for (const opt of paramType)
                sel.add(new Option(opt, opt));
            if (storedValues[paramName])
                sel.value = storedValues[paramName];
            row.appendChild(sel);
        }
...
function buildColorInput(typeVal, colorVal) {
    ...
    const renderDynamic = () => {
        ...
        if (typeSelect.value === 'theme') {
            ...
            text.value = initVal;
            if (initVal.startsWith('#') && initVal.length === 7)
                picker.value = initVal;
...
const targetContainer = document.getElementById('settings-container');
...
function buildUI() {
    ...
    document.getElementById('canvas_color_picker')
        .addEventListener('input', (e) => {
        document.getElementById('canvas_color').value =
            e.target.value;
    });
...
function addCustomRuleCard(ruleData) {
    ...
    card.querySelector('.cr-color-container')
    ...
    card.querySelector('.cr-name').addEventListener('input', (e) => {
        card.querySelector('.rule-title-display').textContent =
            e.target.value || 'Unnamed Rule';
    });
    ...
    const updateVisibility = () => {
        if (modeSelect.value === 'simple') {
            ...
            if (ch)
                base[`${prefix}_asym_id`] = ch;
            if (at)
                base[`${prefix}_atom_id`] = at;
            if (el)
                base['type_symbol'] = el;
            const selectors = [];
            ...
            if (specific)
                specific.split(',').forEach(p => {
                    if (!isNaN(parseInt(p)))
                        selectors.push({ ...base, [`${prefix}_seq_id`]: parseInt(p) });
                });
            if (idxs)
                idxs.split(',').forEach(p => {
                    if (!isNaN(parseInt(p)))
                        selectors.push({ ...base, atom_index: parseInt(p) });
                });
            if (selectors.length === 0 && Object.keys(base).length > 0)
                selectors.push(base);
...
function injectSettingsIntoUI(settingsObj) {
    ...
    if (settingsObj.canvas_color) {
        ...
        if (settingsObj.canvas_color.startsWith('#'))
            pickerInput.value = settingsObj.canvas_color;
    }
    ...
    for (const target of AppConfig.targets) {
        ...
        if (repVal) {
            ...
            repSel.value = repVal;
            updateSubParamsDrawer(card.querySelector('.target-drawer'), repVal, settingsObj[`${target.id}_subParams`] ?? {});
        }
        ...
        if (sizeVal)
            card.querySelector('.size-input').value = sizeVal;
        if (opacityVal)
            card.querySelector('.opacity-input').value = opacityVal;
    }
...
document.getElementById('delete-template')?.addEventListener('click', () => {
    ...
    if (confirm(`Delete "${customTemplates[id]?.name}"?`)) {
        ...
        StorageAPI.set({ customTemplates: customTemplates }, () => {
...
document.getElementById('save-template')?.addEventListener('click', () => {
    ...
    const existingId = Object.keys(customTemplates).find(k => customTemplates[k].name.toLowerCase() === name.toLowerCase());
    if (existingId && !confirm(`Overwrite "${name}"?`))
        return;
...
document.getElementById('export-json')?.addEventListener('click', () => {
    ...
    a.setAttribute('href', 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(extractCurrentSettings(), null, 2)));
    ...
    reader.onload = (evt) => {
        try {
            ...
            if (Array.isArray(parsed.customRules)) {
                safe.customRules = parsed.customRules
