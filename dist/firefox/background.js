...
chrome.contextMenus.onClicked.addListener((info, _tab) => {
    ...
    const viewerUrl = chrome.runtime.getURL(`viewer.html?fileUrl=${encodeURIComponent(info.linkUrl)}&format=unknown`);
...
chrome.runtime.onMessage.addListener((message, sender) => {
    ...
    if (!message.url || !isSafeUrl(message.url))
        return;
    if (!ALLOWED_FORMATS.has(message.format))
        return;
    const viewerUrl = chrome.runtime.getURL(`viewer.html?fileUrl=${encodeURIComponent(message.url)}&format=${encodeURIComponent(message.format)}`);
    chrome.tabs.create({ url: viewerUrl });
});
// ---------------------------------------------------------------------------
...
chrome.permissions.onAdded.addListener((permissions) => {
    ...
    if (origins.length === 0 || !chrome.scripting?.registerContentScripts)
        return;
