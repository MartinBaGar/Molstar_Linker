...
export const MvsBuilder = {
    ...
    _buildComponentBranches(settings) {
        ...
        customRules.slice(0, MAX_RULES).forEach(rule => {
            ...
            if (!safeSelector)
                return; // A rule without a selector is meaningless
            if (safeLabel)
        ...
        for (const target of AppConfig.targets) {
            ...
            this._applySafeSubParams(repParams, repParams.type, settings[`${target.id}_subParams`]);
            const repChildren = [
                this._getColorNode(settings[`${target.id}_colorType`], settings[`${target.id}_colorVal`]),
            ];
