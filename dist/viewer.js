...
function isSafeUrl(urlStr) {
    try {
        ...
        if (!ALLOWED_URL_SCHEMES.has(u.protocol))
            return false;
        if (BLOCKED_RANGES.some(r => r.test(u.hostname)))
            return false;
        return true;
    }
    catch {
...
function spawnIframe(dataUri, format, rawUrl) {
    ...
    extApi.storage.sync.get(AppConfig.getDefaults(), (storedSettings) => {
        ...
        const messageListener = (e) => {
            if (e.data?.action !== 'SANDBOX_READY' || e.source !== iframe.contentWindow)
                return;
...
async function bootWorkspace(rawUrl, safeFormat) {
    const loadingDiv = document.getElementById('loading');
    if (loadingDiv)
        loadingDiv.innerText = 'Downloading structure securely…';
    try {
        ...
        if (preview.trim().startsWith('<?xml') || preview.includes('<Error>')) {
            throw new Error('Download blocked by browser tracking protection. ' +
...
function setupDragAndDrop() {
    ...
    window.addEventListener('drop', (e) => {
        ...
        let format = ext;
        if (ext === 'ent')
            format = 'pdb'; // .ent is a PDB alias
        if (ext === 'cif')
            format = 'mmcif'; // bare .cif → mmcif parser
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!rawUrl) {
        if (loadingDiv)
            loadingDiv.innerText = 'Loading empty workspace…';
    ...
    if (!isSafeUrl(rawUrl)) {
        if (loadingDiv)
            loadingDiv.innerText = 'Error: request to unsafe or restricted URL blocked.';
        return;
    }
    ...
    if (!ALLOWED_FORMATS.has(format)) {
        ...
        if (!isDefault) {
            ...
            if (!storageData.customDomains.includes(targetDomain)) {
                if (loadingDiv)
                    showUnauthorizedDomainUI(loadingDiv, targetDomain);
                return;
            }
        }
        if (loadingDiv)
            showFormatSelectorUI(loadingDiv, rawUrl);
        return;
    }
